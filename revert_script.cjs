...
const target = `      // 1. Base Color Layer
      ctx.save();
      ctx.filter = 'blur(1px)';
...
const replacement = `      // Source-over gives stronger color coverage, multiply restores texture,
...
if (content.includes(normalizedTarget)) {
    ...
    console.log('Could not find target content to revert.');
}
